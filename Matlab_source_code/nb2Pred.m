...
if newPred{5,1} == 'W'
    newPred{5,1} = 2;
elseif newPred{5,1} == 'L'
    newPred{5,1} = 0;
elseif newPred{5,1} == 'D'
    newPred{5,1} = 1;
end
...
if newPred{6,1} == 'Y'
    newPred{6,1} = 1;
elseif newPred{6,1} == 'N'
    newPred{6,1} = 0;
end

testNewPred = newPred{2:6, end}; %assign the new prediction test point
...
nbc(:,1)=[];
classVariable = nbc(:,6); %get our class variable
nbc = nbc(:,1:5)'; %features are now displayed correct way

classVariableW = classVariable; %new array to begin first binary classification
...
classVariableD = classVariable; %new array to begin second binary classification
